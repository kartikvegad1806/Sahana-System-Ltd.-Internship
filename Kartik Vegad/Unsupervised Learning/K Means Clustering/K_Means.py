...
class DatasetLoader:
    ...
    def _generate_synthetic_dataset(self, n_samples: int = 600) -> pd.DataFrame:
        """
        Generate synthetic mall customer dataset with realistic segments.
        """
        ...
        # Segment 1: Young high spenders
        age = np.random.randint(18, 30, sizes[0])
        income = np.random.randint(40, 80, sizes[0])
        score = np.random.randint(70, 100, sizes[0])

        for a, i, s in zip(age, income, score):
            records.append({
                'Gender': np.random.choice(genders),
                'Age': int(a),
                'Annual Income (k$)': int(i),
                'Spending Score (1-100)': int(s),
                'TrueCluster': 0
            })

        # Segment 2: Middle income moderate spenders
        age = np.random.randint(30, 50, sizes[1])
        income = np.random.randint(40, 70, sizes[1])
        score = np.random.randint(40, 70, sizes[1])
        ...
        # Segment 3: High income low spenders
        age = np.random.randint(35, 60, sizes[2])
        income = np.random.randint(70, 120, sizes[2])
        score = np.random.randint(10, 40, sizes[2])
        ...
        age = np.random.randint(25, 45, sizes[3])
        income = np.random.randint(80, 140, sizes[3])
        score = np.random.randint(70, 100, sizes[3])
        ...
        df.insert(0, 'CustomerID', np.arange(1, len(df) + 1))
...
class IncomeVisualizer:
    ...
    def plot_missing_values(self):
        ...
        plt.tight_layout()
        plt.savefig("K Means Clustering\\graphs\\income_missing_values.png", dpi=DPI, bbox_inches='tight')
        plt.close()
    ...
    def plot_feature_distributions(self):
        ...
        plt.tight_layout()
        plt.savefig("K Means Clustering\\graphs\\income_feature_distributions.png", dpi=DPI, bbox_inches='tight')
        plt.close()
        print("[OK] Feature distributions saved")

    def plot_age_vs_income(self):
        plt.figure(figsize=(10, 6), dpi=DPI)
        plt.scatter(self.data['Age'], self.data['Annual Income (k$)'],
        ...
        plt.tight_layout()
        plt.savefig("K Means Clustering\\graphs\\income_age_vs_income_raw.png", dpi=DPI, bbox_inches='tight')
        plt.close()
        print("[OK] Age vs Income (raw) saved")

    def plot_income_boxplot(self):
        plt.figure(figsize=(8, 5), dpi=DPI)
        plt.boxplot(self.data['Annual Income (k$)'], vert=True, patch_artist=True,
                    boxprops=dict(facecolor='#1f77b4', alpha=0.7))
        plt.title("Income Distribution (Boxplot)", fontsize=12, fontweight='bold')
        plt.ylabel("Annual Income (k$)")
        plt.grid(True, alpha=0.3, axis='y')
        plt.tight_layout()
        plt.savefig("K Means Clustering\\graphs\\income_boxplot.png", dpi=DPI, bbox_inches='tight')
        plt.close()
    ...
    def plot_age_boxplot(self):
        ...
        plt.grid(True, alpha=0.3, axis='y')
        plt.tight_layout()
        plt.savefig("K Means Clustering\\graphs\\income_age_boxplot.png", dpi=DPI, bbox_inches='tight')
        plt.close()
    ...
    def plot_correlation_heatmap(self):
        ...
        plt.tight_layout()
        plt.savefig("K Means Clustering\\graphs\\income_correlation_heatmap.png", dpi=DPI, bbox_inches='tight')
        plt.close()
...
class ModelEvaluator:
    ...
    def _plot_clusters(self, X_raw: np.ndarray, df: pd.DataFrame):
        ...
        plt.tight_layout()
        plt.savefig("K Means Clustering\\graphs\\income_cluster_result.png", dpi=DPI, bbox_inches='tight')
        plt.close()
    ...
    def _plot_centroids(self, X_raw: np.ndarray, X_scaled: np.ndarray):
        ...
        for idx, centroid in enumerate(c_orig):
        ...
        ax2.set_ylabel('Annual Income k$ (centroid)', color='#ff7f0e')
        ...
        plt.tight_layout()
        plt.savefig("K Means Clustering\\graphs\\income_centroids.png", dpi=DPI, bbox_inches='tight')
        plt.close()
    ...
    def _plot_cluster_distributions(self, df: pd.DataFrame):
        ...
        for cl in range(self.model.n_clusters):
            mask = self.model.labels_ == cl
            axes[0][1].hist(df.loc[mask, 'Annual Income (k$)'].values, bins=15,
                            alpha=0.6, label=f'Cluster {cl}',
                            color=CLUSTER_COLORS[cl], edgecolor='black')
        ...
        cluster_data_inc = [df.loc[self.model.labels_ == cl, 'Annual Income (k$)'].values
                            for cl in range(self.model.n_clusters)]
        ...
        plt.tight_layout()
        plt.savefig("K Means Clustering\\graphs\\income_cluster_distributions.png", dpi=DPI, bbox_inches='tight')
        plt.close()
    ...
    def _plot_pca_projection(self, X_scaled: np.ndarray):
        ...
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.savefig("K Means Clustering\\graphs\\income_pca_clusters.png", dpi=DPI, bbox_inches='tight')
        plt.close()
...
class MLPipeline:
    ...
    def run(self):
        ...
        output_path = "K Means Clustering\\income_clustered.csv"
    ...
    def _find_optimal_k(self, X_scaled: np.ndarray) -> int:
        ...
        plt.tight_layout()
        plt.savefig("K Means Clustering\\graphs\\income_optimal_k.png", dpi=DPI, bbox_inches='tight')
        plt.close()
    ...
    def _print_cluster_summary(self, df: pd.DataFrame, X_raw: np.ndarray):
        ...
        for cl in sorted(df['Cluster'].unique()):
            ...
            print(f"    Income  : min={subset['Annual Income (k$)'].min():,}  "
                  f"max={subset['Annual Income (k$)'].max():,}  "
                  f"mean={subset['Annual Income (k$)'].mean():,.0f}")
    ...
    def _predict_new_person(self):
        ...
        new_person = {'Age': 38, 'Annual Income (k$)': 72, 'Spending Score (1-100)': 75}
        print("New person profile:")
        for k, v in new_person.items():
            print(f"  {k}: {v}")

        X_new       = np.array([[new_person['Age'], new_person['Annual Income (k$)'], new_person['Spending Score (1-100)']]])
